...
# New parameters for smoother turning
TURN_SMOOTHING_BUFFER_SIZE = 3
TURN_SPEED_STEPS = [50, 100, 150, 200, 255]  # Predefined turn speed steps

turn_positions = deque(maxlen=TURN_SMOOTHING_BUFFER_SIZE)
previous_turn_command = None
...
# Deadband helper for serial command updates
...
def smooth_turning(object_center_x):
    """
    Implement a more nuanced turning strategy with gradual speed changes
    """
    if object_center_x < SECTION_WIDTH:
        # Interpolate turn speed based on how far left the object is
        turn_intensity = 1 - (object_center_x / SECTION_WIDTH)
        turn_speed_index = min(int(turn_intensity * len(TURN_SPEED_STEPS)), len(TURN_SPEED_STEPS) - 1)
        return f"TURN LEFT {TURN_SPEED_STEPS[turn_speed_index]}"
    elif object_center_x > 3 * SECTION_WIDTH:
        # Interpolate turn speed based on how far right the object is
        turn_intensity = (object_center_x - 3 * SECTION_WIDTH) / SECTION_WIDTH
        turn_speed_index = min(int(turn_intensity * len(TURN_SPEED_STEPS)), len(TURN_SPEED_STEPS) - 1)
        return f"TURN RIGHT {TURN_SPEED_STEPS[turn_speed_index]}"
    return "STOP"

# Smooth Transition Function
def smooth_transition(new_command, old_command):
    """
    Add smoothing to command transitions to prevent abrupt changes
    """
    if new_command.startswith("TURN"):
        # Add the new turn command to a buffer
        turn_positions.append(new_command)
        
        # If we have enough historical data, average the commands
        if len(turn_positions) == TURN_SMOOTHING_BUFFER_SIZE:
            # Count occurrences of left and right turns
            left_turns = sum(1 for cmd in turn_positions if "LEFT" in cmd)
            right_turns = sum(1 for cmd in turn_positions if "RIGHT" in cmd)
            
            # Determine predominant turn direction
            if left_turns > right_turns:
                avg_speed = int(sum(int(cmd.split()[-1]) for cmd in turn_positions) / len(turn_positions))
                return f"TURN LEFT {avg_speed}"
            elif right_turns > left_turns:
                avg_speed = int(sum(int(cmd.split()[-1]) for cmd in turn_positions) / len(turn_positions))
                return f"TURN RIGHT {avg_speed}"
...
def process_frame(frame, arduino):
    ...
    try:
        ...
        if len(indices[0]) > 0:
            ...
            # New Smoothed Turning Logic
            if object_center_x < SECTION_WIDTH or object_center_x > 3 * SECTION_WIDTH:
                movement = smooth_turning(object_center_x)
            ...
            # Apply smooth transition to the command
            movement = smooth_transition(movement, previous_command)

        # Only send command if it has changed (with deadband filtering)
