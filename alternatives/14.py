...
# Moving average filter for smoother tracking (for x position)
SMOOTHING_BUFFER_SIZE = 5
object_positions = deque(maxlen=SMOOTHING_BUFFER_SIZE)

# Moving average filter for smoothing object area measurement
object_area_values = deque(maxlen=SMOOTHING_BUFFER_SIZE)
...
MIN_SPEED = 30       # Minimum motor speed to overcome static friction
...
# Adaptive Color Thresholding in HSV with lighting improvements
def process_image(image):
    try:
        # Convert RGB to HSV for robust color detection under varying lighting
        hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
        # Apply histogram equalization on the V channel to correct brightness
...
def process_frame(frame, arduino):
    ...
    try:
        ...
        if len(indices[0]) > 0:
            ...
            # Approximate object size based on pixel count
            object_area = len(indices[0])
            
            # Smooth object center for stability
            object_center_x = smooth_position(object_center_x)
            # ----------------- New: Smooth the object area -----------------
            object_area_values.append(object_area)
            smoothed_area = int(np.mean(object_area_values))
            ...
            if object_center_x < SECTION_WIDTH:
                ...
                # Use PID controller to set speed smoothly using the smoothed area
                speed_output = pid_controller.update(smoothed_area)
                # Clamp the speed to within allowed limits
                if speed_output < 0:
                    speed_output = 0
                if speed_output > MAX_SPEED:
                    speed_output = MAX_SPEED
                # Enforce a minimum speed if a positive command is given
