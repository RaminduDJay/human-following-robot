...
# Object Color Range (Adjust for object color)
...
def process_frame(frame, arduino):
    ...
    try:
        ...
        # Apply noise reduction with simple averaging filter
        image = (image[:-1] + image[1:]) // 2  # Basic noise reduction
        ...
        if len(indices[0]) > 0:
            ...
            # Adaptive movement thresholds (adjust dynamically)
            global FORWARD_THRESHOLD, STOP_THRESHOLD
            FORWARD_THRESHOLD = max(3000, int(object_area * 0.5))
            STOP_THRESHOLD = max(6000, int(object_area * 0.8))
